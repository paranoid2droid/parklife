...
Project is in maintenance + enrichment mode. **461 parks / 9,579 visible species / 126,405 visible park-species pairs** as of 2026-05-30. Code + Pages site at <https://github.com/paranoid2droid/parklife>; demo published from `docs/` at <https://paranoid2droid.github.io/parklife/> (current export 43.0 MB). **5,286 species** have curated profiles in ja/en/zh/zhT. **np≥2 tiers 100% CLEARED** (np≥3 done 2026-05-31; np=2 done A104–A149 on 2026-06-01, verified 0). **np=1 tier IN PROGRESS** (A150–A162 done; 2026-06-04 session ran A157–A162, +144; 5,286 profiled, **1,734 np=1 candidates remaining**; next A163 from Bipalium kewense). P13 official-URL coverage **443/461 parks (96%)** — 18 small 緑地/河川敷 stay `__no_url__`. Parking: 122 OSM-only + 339 text-confirmed.
...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A162 done)** *(at 5,286 / 2026-06-04; 1,734 np=1 candidates remaining)*
   - **Next batch A163 — the verified clean top-24 (np=1, from A162done.txt): Bipalium kewense, Bischofia javanica, Bistolida hirundo, Biston regalis, Biwia yodoensis, Bizia aexaria, Bjerkandera fumosa, Blemus discus, Blyxa japonica, Boehmeria holosericea, Boerhavia erecta, Boletellus emodensis, Boletellus obscurecoccineus, Boletus nigromaculatus, Boninogaster phalloides, Bonomyces sinopicus, Brachidontes mutabilis, Brachymenium nepalense, Brachytron longistigma, Bradina angustalis, Brahmaea wallichii, Branchiostegus japonicus, Brasenia schreberi, Brassica oleracea.** Use `/tmp/finish_np1.sh` + robust-rekey model (latest `/tmp/profile_batchA162.py`). Dump→READ→copy names EXACTLY; verify profile_count +N AND remain −N.
   - **A163 DB-junk to fix first** (in `/tmp/fixesA163.sql`): ja kanji→katakana `Boehmeria holosericea` 仁王藪苧麻→ニオウヤブマオ, `Boletellus obscurecoccineus` 深山紅猪口→ミヤマベニイグチ; en junk `Bischofia javanica` "kainfal"→Autumn Maple Tree, `Blyxa japonica` "Oriental bamboo plant"(wrong; it's a submerged aquatic)→Japanese Blyxa, `Boerhavia erecta` "Hogmeat"→Erect Boerhavia, `Boletellus emodensis` "shaggy cap"→Shaggy Bolete, `Bonomyces sinopicus` "spring funnel"→Sinopica Funnel, `Branchiostegus japonicus` "Horse Head"→Japanese Horsehead Tilefish, `Brassica oleracea`(ヤセイカンラン=wild cabbage) "Kale"→Wild Cabbage.
   - **⚠️ PUSH NOTE (2026-06-04):** the 43 MB `docs/index.html` push now sometimes times out (`pushed_exit=128`, "server github.com not responding"). `finish_np1.sh` commits locally even when push fails — **after each batch confirm `git status -sb` shows NOT "ahead", and if it is, just rerun `git push`** (A162's commit 5f9761f needed a manual retry).
