...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A196 done)** *(at 6,100 / 2026-06-10; 918 np=1 candidates remaining)*
   ...
   - **Next batch A197 — verified clean top-24 (np=1, from DB query, SKIPPING Latona bicolor): Lespedeza tomentosa, Lethrinus nebulosus, Leucaena leucocephala, Leucauge subblanda, Leucoinocybe auricoma, Leucophaeus pipixcan, Leucospis japonica, Leucosticte arctoa, Ligdia ciliaria, Ligustrum micranthum, Lilium formosana, Lilium longiflorum, Lima vulgaris, Limnodromus semipalmatus, Limnophila indica, Limonius eximius, Limosella aquatica, Linaria maroccana, Linaria purpurea, Linckia guildingi, Lindera aggregata, Lindera sericea, Liquidambar formosana, Liquidambar styraciflua.** (Re-run the np=1 query for fresh en detail before writing. Notables: Leucaena leucocephala ギンネム = invasive lead tree (en junk "vi-vi"→fix); Leucophaeus pipixcan = Franklin's Gull + Leucosticte arctoa ハギマシコ Rosy Finch + Limnodromus semipalmatus Asian Dowitcher = birds (+eBird); both Lilium formosana & longiflorum = テッポウユリ Easter lily (homonym ja — verify); both Linaria maroccana & purpurea = ヒメキンギョソウ (homonym ja); Liquidambar formosana フウ & styraciflua モミジバフウ = sweetgums (en "Formosan-gum"/"redgum"→fix caps); Limosella aquatica キタミソウ = rare mudwort; Lethrinus nebulosus = spangled/blue emperor (food fish).)
