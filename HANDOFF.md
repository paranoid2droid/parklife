...
Project is in maintenance + enrichment mode. **461 parks / 9,579 visible species** as of 2026-05-30. Code + Pages site at <https://github.com/paranoid2droid/parklife>; demo published from `docs/` at <https://paranoid2droid.github.io/parklife/> (export ~44 MB). **5,692 species** have curated profiles in ja/en/zh/zhT. **np≥2 tiers 100% CLEARED**. **np=1 tier IN PROGRESS** (A150–A179 done; 2026-06-05/06 session ran A157–A179, +550; 5,692 profiled, **1,326 np=1 candidates remaining**; next A180 from Dryopteris mayebarae). **Sidecar data (`data/species_profiles_extra.json`) is pushed & current via the sidecar-only commit workflow.** ⚠️ **`docs/index.html` (published demo) LAGS at ~A174 data**: the 44 MB file's push keeps timing out on this connection (intermittent throttle; `ls-remote` works). The regenerated index.html is **uncommitted in the working tree** — to publish: `git add docs/index.html && git commit && git push` and **retry the push until a window catches it** (worked twice on 2026-06-05). No curation data is lost — all profiles are in the pushed sidecar. P13 official-URL coverage **443/461 parks (96%)**. Parking: 122 OSM-only + 339 text-confirmed.
...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A179 done)** *(at 5,692 / 2026-06-06; 1,326 np=1 candidates remaining)*
   - **Next batch A180 — verified clean top-24 (np=1, from DB query): Dryopteris mayebarae, Dryopteris monticola, Dryopteris saxifragivaria, Dryopteris sieboldii, Dryopteris subhikonensis, Dryopteris tokyoensis, Dryopteris varia, Dryotribus mimeticus, Duranta erecta, Dysaethria meridiana, Echinaster luzonicus, Echinophyllia aspera, Echinops ritro, Ectropis crepuscularia, Egiona picta, Ehretia dicksonii, Elaeagnus rotundata, Elaeagnus thunbergii, Elaeocarpus zollingeri, Elaphropus latissimus, Elasmostethus humeralis, Eleocharis kamtschatica, Eleotris oxycephala, Eleusine coracana.** (7 Dryopteris ferns lead it.)
   ...
   - **⚠️ `docs/index.html` may be UNCOMMITTED in working tree** (regenerated through A177 but its 43.9 MB push kept failing this session). To publish the demo: `git add docs/index.html && git commit -m "docs: regenerate index.html" && git push` — **retry the push repeatedly; it lands in a good window.** All sidecar/profile data is already on remote, so the demo just lags until this pushes.
   - **⚠️⚠️ PUSH LESSON (2026-06-05):** committing the 43.5 MB `docs/index.html` EVERY batch makes packs huge (5 commits = **77 MB**) and the throttled upload disconnects mid-sideband (`Connection to …github.com port 22/443 timed out`; `ls-remote` still works → it's the data upload, not connectivity). Sidecar-only commits (tens of KB) push fine. Keep `docs/index.html` out of per-batch commits; push it occasionally with retries.
...
### 2026-06-06 (Claude/Opus) — np=1: A178–A179 (+48 →5,692)
- A178 Dichomeris tostella→Distoleon nigricans (porcupinefish/toxic-berry warnings), A179 Ditrema temminckii→Dryopteris maximowiczii (incl. アマガエル Japanese tree frog, beard lichens, Drupa rock snails). 1,326 np=1 remain. **Next A180 from Dryopteris mayebarae.** All sidecar pushed clean; `docs/index.html` still uncommitted/lagging (see Status).
