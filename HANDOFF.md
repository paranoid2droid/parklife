...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150 done)** *(at 5,048 / 2026-06-01; ~1,970 np=1 candidates remaining)*
   - **Next batch A151 — the verified clean top-24 (np=1, copied from A150's query output) are: Achnatherum extremiorientale, Acmaeops septentrionis, Acosmeryx castanea, Acrida cinerea, Acrida willemsei, Acrocercops transecta, Actaea asplenifolia, Actinostola sp., Aculops niphocladae, Aderidae, Adscita sp., Aedes galloisi, Aeluropus littoralis, Aeschynomene indica, Agapanthus praecox, Agarum clathratum, Agasicles hygrophila, Aglaomorpha coronans, Agrilus chujoi, Agriosphodrus dohrni, Agropyron racemiferum, Agrostis clavata, Ahnfeltiopsis flabelliformis, Aiolocaria hexaspilota.** Use the np=1 finish script `/tmp/finish_np1.sh` (HAVING `=1`) + robust-rekey batch model (`/tmp/profile_batchA150.py`). Dump→READ→copy names EXACTLY; verify profile_count +N. **np=1 ≈ 1,994 / ~83 batches — multi-session marathon; pace it, keep HANDOFF current.**
   - **DB-junk watch (seen in np=1 already):** kanji ja-names (Abies mariesii 大白檜曽→オオシラビソ), en "Animal"/lowercase/odd ("Bele","convictfish","Sergeants"), HTML-entity & romaji-junk zh aliases (`大鳍&#x...;`, `毛果枫 (Mao Guo Feng)`, `Mak mo`) — fix in DB (`/tmp/fixesA150.sql` is a model; the `DELETE FROM species_alias WHERE raw_name LIKE '%&#x%'` purge is handy) before profiling.
