...
Project is in maintenance + enrichment mode. **461 parks / 9,579 visible species / 126,405 visible park-species pairs** as of 2026-05-30. Code + Pages site at <https://github.com/paranoid2droid/parklife>; demo published from `docs/` at <https://paranoid2droid.github.io/parklife/> (current export 43.4 MB). **5,430 species** have curated profiles in ja/en/zh/zhT. **np≥2 tiers 100% CLEARED** (np≥3 done 2026-05-31; np=2 done A104–A149 on 2026-06-01, verified 0). **np=1 tier IN PROGRESS** (A150–A168 done; 2026-06-04/05 session ran A157–A168, +288; 5,430 profiled, **1,590 np=1 candidates remaining**; next A169 from Chamaegastrodia shikokiana). P13 official-URL coverage **443/461 parks (96%)** — 18 small 緑地/河川敷 stay `__no_url__`. Parking: 122 OSM-only + 339 text-confirmed.
...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A168 done)** *(at 5,430 / 2026-06-05; 1,590 np=1 candidates remaining)*
   - **Next batch A169 — the verified clean top-24 (np=1, from A168done.txt): Chamaegastrodia shikokiana, Chamaemelum nobile, Charadrius leschenaultii, Charadrius veredus, Charagochilus longicornis, Charitographa mikadonis, Chascanopsetta lugubris, Chelidonichthys kumu, Chelidonura hirundinina, Chelonomorpha japana, Chenopodium quinoa, Chilo suppressalis, Chionarctia nivea, Chionographis japonica, Chiua virens, Chlidonias leucopterus, Chloris gayana, Chloris radiata, Chloris truncata, Chlorurus microrhinos, Chlorurus spilurus, Choerades nigrovittata, Choerodon azurio, Chonostropheus chujoi.** Use `/tmp/finish_np1.sh` + robust-rekey model (latest `/tmp/profile_batchA168.py`). Dump→READ→copy names EXACTLY; verify profile_count +N AND remain −N.
   - **A169 DB-junk to fix first** (in `/tmp/fixesA169.sql`): en junk `Charadrius leschenaultii` "great sand plover"→Greater Sand Plover (cap), `Chilo suppressalis` "Rice steam borer"(typo)→Asiatic Rice Borer, `Chloris truncata` "star grass"→Windmill Grass (cap). Check `Charadrius leschenaultii`/`Chloris gayana` are just capitalisation. Most en here are OK.
   - **⚠️ PUSH NOTE:** the 43 MB `docs/index.html` push sometimes times out (`pushed_exit=128`, "server github.com not responding"). `finish_np1.sh` commits locally even when push fails — **after each batch confirm `git status -sb` shows NOT "ahead", and if it is, rerun `git push`** (happened once on A162; A163–A168 all pushed clean).
