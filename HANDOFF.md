...
Project is in maintenance + enrichment mode. **461 parks / 9,579 visible species** as of 2026-05-30. Code + Pages site at <https://github.com/paranoid2droid/parklife>; demo published from `docs/` at <https://paranoid2droid.github.io/parklife/> (export ~43.9 MB). **5,644 species** have curated profiles in ja/en/zh/zhT. **np≥2 tiers 100% CLEARED**. **np=1 tier IN PROGRESS** (A150–A177 done; 2026-06-04/05 session ran A157–A177, +502; 5,644 profiled, **1,374 np=1 candidates remaining**; next A178 from Dichomeris tostella). **Sidecar data (`data/species_profiles_extra.json`) is pushed & current.** ⚠️ **`docs/index.html` (the published demo) may lag**: the 43.9 MB file pushes unreliably on this connection — it is regenerated locally but committed+pushed separately (see PUSH LESSON in Active #1); check `git status` and retry `git push` for it. P13 official-URL coverage **443/461 parks (96%)**. Parking: 122 OSM-only + 339 text-confirmed.
...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A177 done)** *(at 5,644 / 2026-06-05; 1,374 np=1 candidates remaining)*
   - **Next batch A178 — verified clean top-24 (np=1, from DB query): Dichomeris tostella, Dichomeris ustalella, Dicranum hamulosum, Dicranum japonicum, Dictyestra dissectus, Dictyna arundinacea, Dictyonotus purpurascens, Didea alneti, Dinebra retroflexa, Diocalandra sasa, Diodon holocanthus, Diodon hystrix, Diomea cremata, Diphyscium lorifolium, Diplacrum caricinum, Diplazium nipponicum, Diplazium takii, Diplocalyptis congruentana, Diplocephaloides saganus, Diplocyclos palmatus, Dipsacus japonicus, Distachys vulgaris, Distimake tuberosus, Distoleon nigricans.** Note: Diodon holocanthus/hystrix = porcupinefish (puffer relatives, can carry TTX — warn).
