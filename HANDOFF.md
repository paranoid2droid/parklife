...
1. **species_profile curation — 🎉🎉 COMPLETE. Entire profilable backlog cleared (A1–A233 done).** *(~7,007 / 2026-06-17; only 7 genus-level 属 stubs remain — irreducible)*
   ...
   - **➡️ NEXT WORK = pivot off profiling.** Options: (a) **regenerate + publish the demo** — it lags many batches behind (all the A150–A233 profiles + the local DB name-fixes/merge/deletes only reach the public site after `export_html` → `cp docs/` → push the ~46 MB `parklife-data.json`); (b) Active TODO item 2 (latent-data maintenance scripts); (c) the productization / national-expansion track in agent memory. Ask the user which.
