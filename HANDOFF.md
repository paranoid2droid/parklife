...
Project is in maintenance + enrichment mode. **461 parks / 9,579 visible species** as of 2026-05-30. Code + Pages site at <https://github.com/paranoid2droid/parklife>; demo published from `docs/` at <https://paranoid2droid.github.io/parklife/> (current export ~43.6 MB). **5,525 species** have curated profiles in ja/en/zh/zhT. **np≥2 tiers 100% CLEARED**. **np=1 tier IN PROGRESS** (A150–A172 done; 2026-06-04/05 session ran A157–A172, +383; 5,525 profiled, **1,494 np=1 candidates remaining**; next A173 from Conocephalus halophilus). **⚠️ 2026-06-05 GitHub push outage:** A171+A172 commits (20cf213, b2c6b68) are LOCAL ONLY — pushes to github.com timed out repeatedly (`pushed_exit=128`). The DB/docs/sidecar are all correct locally; **next agent: run `git push` until `git status` is clean before doing anything else.** P13 official-URL coverage **443/461 parks (96%)**. Parking: 122 OSM-only + 339 text-confirmed.
...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A172 done)** *(at 5,525 / 2026-06-05; 1,494 np=1 candidates remaining)*
   - **FIRST: `git push` until clean** (A171+A172 pending due to GitHub outage — see Status).
   - **Next batch A173 — verified clean top-24 (np=1, from DB query): Conocephalus halophilus, Conocybe fragilis, Conotalopia ornata, Conothele fragaria, Conus arenatus, Conus boeticus, Conus catus, Conus chaldaeus, Conus ebraeus, Conus fulgetrum, Conus lividus, Conus miles, Conus omaria, Conus rattus, Convallaria keiskei, Convallaria majalis, Coprinopsis cinerea, Coprinopsis strossmayeri, Coptis japonica var. dissecta, Corbicula fluminea, Corbicula sandai, Corbula venusta, Corgatha dictaria, Corgatha pygmaea.** ⚠️ **10 Conus (cone snails) — ALL venomous (harpoon radular tooth); warn "do not handle live shells", some can be dangerous/deadly.** Use `/tmp/finish_np1.sh` + robust-rekey model (latest `/tmp/profile_batchA172.py`).
   - **A173 DB-junk to fix first** (`/tmp/fixesA173.sql`): capitalise lowercase Conus en (worm cone→Worm Cone, rat cone→Rat Cone, soldier cone→Soldier Cone, Cat cone→Cat Cone, Hebrew cone→Hebrew Cone, Livid cone→Livid Cone, Omaria cone→Omaria Cone); `Convallaria majalis` ja kanji 君影草→ドイツスズラン (keiskei already=スズラン; both toxic — convallatoxin, warn). 
   - **⚠️ PUSH NOTE:** the ~43 MB `docs/index.html` push intermittently times out (`pushed_exit=128`). `finish_np1.sh` commits locally even when push fails — **after each batch confirm `git status -sb` is NOT "ahead"; if it is, rerun `git push`** (single push sends all stacked commits). 2026-06-05 had a sustained outage stacking 2 commits.
...
### 2026-06-05 (Claude/Opus) — np=1 tier continued: A169–A172 (5,430→5,525, +95; A171/A172 push-pending)
