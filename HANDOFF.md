...
Project is in maintenance + enrichment mode. **461 parks / 9,641 visible species / 126,586 visible park-species pairs** as of commit `3aae6e9` (2026-05-25). Code + Pages site at <https://github.com/paranoid2droid/parklife>; demo published from `docs/` at <https://paranoid2droid.github.io/parklife/> (current export 33.8 MB). **2,009 species** have curated profiles in ja/en/zh/zhT (8,036 rows), up from 1,987 at the previous batch's end (+22 this batch via A11). `common_name_en` coverage on profiled species is 97% (18 NULL); `zh-Hans` alias coverage 96% (37 missing, of which 26 are alias-collision with synonymous species). P13 official-URL coverage: **443/461 parks (96%)** — only 18 small 緑地/河川敷 remain `__no_url__` after manual investigation. Parking classification: 122 OSM-only (down from 213) + 339 text-confirmed (up from 248).
...
2. **Continue species_profile curation** *(at 2009 / 2026-05-25 — np=12 tier, in progress)*
   - **Current state**: 2,009 species × 4 langs (8,036 rows) curated. Last batch `3aae6e9` (A11). Latest batch added +22 in np=12 tier (mixed amphibian/bird/insect; cleared most large-name candidates at np=12).
