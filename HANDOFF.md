...
Project is in maintenance + enrichment mode. **461 parks / 9,579 visible species** as of 2026-05-30. Code + Pages site at <https://github.com/paranoid2droid/parklife>; demo published from `docs/` at <https://paranoid2droid.github.io/parklife/> (current export ~43.6 MB). **5,573 species** have curated profiles in ja/en/zh/zhT. **np≥2 tiers 100% CLEARED**. **np=1 tier IN PROGRESS** (A150–A174 done; 2026-06-04/05 session ran A157–A174, +431; 5,573 profiled, **1,446 np=1 candidates remaining**; next A175 from Crotalaria sessiliflora). All work pushed & `git status` clean as of 2026-06-05 (the mid-session push outage was resolved — see PUSH lesson in Active #1). P13 official-URL coverage **443/461 parks (96%)**. Parking: 122 OSM-only + 339 text-confirmed.
...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A174 done)** *(at 5,573 / 2026-06-05; 1,446 np=1 candidates remaining)*
   - **Next batch A175 — verified clean top-24 (np=1, from DB query): Crotalaria sessiliflora, Crustodontia chrysocreas, Cryphia mitsuhashi, Cryptarcha strigata, Cryptaspasma angulicostana, Cryptonatica affinis, Cryptonatica janthostoma, Cryptorhynchus lapathi, Ctenophora nohirae, Cucullaearca foliata, Cucumis melo, Cucumis sativus, Cucurbita maxima, Cudoniella clavus, Curculio funebris, Curculio macula-nigra, Curculio yanoi, Cuscuta australis, Cyanistes caeruleus, Cyanthillium cinereum, Cybocephalus nipponicus, Cyclopecten nipponensis, Cyclophorus herklotsi, Cyclosa laticauda.** Note: Crotalaria/Cuscuta toxic; Cucumis/Cucurbita = melon/cucumber/pumpkin crops.
   - **A175 DB-junk to fix** (`/tmp/fixesA175.sql`): en caps/typos — Cryptonatica affinis "Artic Moonsnail"→Arctic Moonsnail, Cryptonatica janthostoma "Kurile moon snail"→Kurile Moonsnail, Cucurbita maxima "Pumkin"→Winter Squash, Crustodontia "Wood decay"→(crust fungus name), Cybocephalus "Beetle"→generic. Most en are OK.
   - **⚠️⚠️ PUSH LESSON (2026-06-05, IMPORTANT):** committing the 43.5 MB `docs/index.html` in EVERY batch makes packs huge (5 commits = **77 MB**) and the upload disconnects on a throttled connection (`Connection to …github.com port 22/443 timed out` mid-sideband — note `ls-remote` still works, so it's the data upload, not connectivity). **FIX/WORKFLOW: use `/tmp/finish_np1_sidecar.sh`** which commits ONLY `data/species_profiles_extra.json` per batch (tiny, pushes instantly). Regenerate + commit + push `docs/index.html` **once per session** (`.venv/bin/python -m scripts.export_html && cp data/export/index.html docs/index.html && git add docs/index.html && git commit && git push`). If the big HTML push disconnects, **just retry `git push` a few times** — it catches a window (worked after ~3 tries on 2026-06-05). Splitting also works: push sidecar/HANDOFF first (small, always succeeds), HTML second.
