...
- Modal now supports previous/next buttons, keyboard arrows, and touch swipe; gallery export uses iNat `large` URLs and renders via `<img>` + `object-fit: contain` (not background-cover) to reduce blur. Regenerated `docs/index.html` (3.8 MB) and `node --check` passed.
