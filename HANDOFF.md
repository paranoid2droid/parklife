...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A207 done)** *(at 6,375 / 2026-06-14; 643 np=1 candidates remaining)*
   - **⚠️ FIVE PLACEHOLDERS at the top of the query — SKIP all every batch:**
     - `Latona bicolor` (ja=動物界 junk; Donacidae bean clam, iNat 1610898) — needs verified katakana ja-name; do NOT profile against 動物界.
     - `Lithospermum murasaki` (ja=ムラサキ, no iNat id) — almost certainly synonym of `Lithospermum erythrorhizon` (profiled A198). **Verify synonymy (POWO/IPNI) and MERGE into erythrorhizon** (repoint observation.species_id, move/clean aliases, delete park_species + species row, re-run `scripts.dedupe`; backup first).
     - `Lunella coronata` (ja=カンギク = winter-chrysanthemum, a PLANT name on a marine turban shell; en="crowned turban shell") — ja-name MISMATCH. Verify correct katakana (likely スガイ-group) and fix DB ja-name before profiling.
     - `Meretrix` (ja=ハマグリ属) and `Monstera adansonii` (ja=モンステラ属) — **genus-level stubs** (属). Skip; not worth bespoke single-species profiles. (Real species like Meretrix lamarckii / Monstera deliciosa were profiled normally.)
   - **Next batch A208 — from Orchestes hustachei** (re-run the np=1 query for fresh top-26 + en detail; alphabet now in the O→P range — many marine inverts, moths, beetles, plus seabirds/ferns).
   - **A199–A207 done (Claude/Opus, 2026-06-13→14):** +227 profiles (6,148→6,375). Pattern each batch: backup DB → apply `/tmp/fixesA2NN.py` (Python+param-binding UPDATEs — handles cultivar quotes; ja-kanji→katakana + wrong/lowercase en, always `WHERE scientific_name=…`) → write `/tmp/profile_batchA2NN.py` (robust-rekey template) → run → `seed_species_profiles` → verify table +N & remain −N → commit **sidecar only** → push. DB backups `/tmp/parklife.db.pre_A199…A207` (en/ja-name fixes are LOCAL-only, regenerated into demo later). Batch sizes 24–26 (taken to a clean genus boundary). Deferrals/stubs accumulated: Lunella coronata (ja mismatch, A199), genus stubs Meretrix (A202) + Monstera adansonii (A203). ~50 en-name fixes + ~6 ja-name fixes total. **CAUTION carried fwd:** count entries before running — A200 I miscounted (wrote 23, added a 24th via a `…b.py` supplement); always confirm `added==len(ENTRIES)`.
