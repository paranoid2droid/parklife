...
Project is in maintenance + enrichment mode. **461 parks / 9,579 visible species / 126,405 visible park-species pairs** as of 2026-05-30. Code + Pages site at <https://github.com/paranoid2droid/parklife>; demo published from `docs/` at <https://paranoid2droid.github.io/parklife/> (current export 42.3 MB). **5,096 species** have curated profiles in ja/en/zh/zhT. **np≥2 tiers 100% CLEARED** (np≥3 done 2026-05-31; np=2 done A104–A149 on 2026-06-01, verified 0). **np=1 tier IN PROGRESS** (A150–A154 done 2026-06-01, swept Abacetus→Acheilognathus+scattered; 5,096 profiled, 1,924 candidates remaining; next A155 from Ainsliaea/Aira); **1,970 candidates remaining**). P13 official-URL coverage **443/461 parks (96%)** — 18 small 緑地/河川敷 stay `__no_url__`. Parking: 122 OSM-only + 339 text-confirmed.
...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A151 done)** *(at 5,049 / 2026-06-01; 1,970 np=1 candidates remaining)*
   - **Next batch A152 — the verified clean top-24 (np=1, from A151's query output): Ajania pacifica, Akodon sp., Alboglossiphonia lata, Aleuria aurantia, Aleurodicus dispersus, Allomyrina dichotoma(?see anomaly), Alpheus brevicristatus, Amalda hilgendorfi, Amblychia angeronaria, Amblyseius womersleyi, Ameiurus melas, Ammophila sabulosa, Ampedus puniceus, Amphidromus…** (re-run the np=1 query to get the full clean 24 — display truncated the tail). Use `/tmp/finish_np1.sh` + robust-rekey model `/tmp/profile_batchA150.py`. Dump→READ→copy names EXACTLY; verify profile_count +N.
