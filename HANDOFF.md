...
1. **species_profile curation — np≥3 DONE. np=2 tier in progress (A104–A142 done; next batch continues alphabetically)** *(at 4,921 / 2026-05-31; 98 np=2 candidates remaining)*
   - **Next batch (A143) start point: around Stenodema calcarata / Stephanitis fasciicarinata onward (Sten…/Step…). Run the Active #1 query (np>=2), dump 24 rows to a file, READ the full file, copy names EXACTLY. DO NOT reconstruct names from memory — that wasted A135/A137/A140. See ⚠️ PROCESS LESSON.**
...
### 2026-05-31 (Claude/Opus) — np=2 tier continued: A139–A142 (4859→4921, +62 net; commits + pushes)
- A139 Russula bella→Scudderocoris, A141 Scyra quadridens→Solenomphala debilis, A142 Sparganium erectum→Stephanitis fasciicarinata (partial 13, display-truncation safe). **A140 was a fabricated-candidate misfire** (built from invented names, not the real list; +0 profiles) — cleaned via re-key/orphan-purge (`/tmp/fix_a140_keys.py`). 98 np=2 candidates remain. **Next A143 from Stenodema/Stephanitis onward.** Demo 41.8→41.9 MB.
