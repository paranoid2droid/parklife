...
- **Result: every profilable visible species is done. Only 7 genus-level 属 stubs remain (irreducible).** Per-batch profiling is over — next agent should pivot (publish lagging demo / maintenance / productization). DB fixes LOCAL (backup /tmp/parklife.db.pre_deferrals). Sidecar pushed.
