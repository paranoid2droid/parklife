...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A216 done)** *(at 6,606 / 2026-06-14; 412 np=1 candidates remaining)*
   - **⚠️ DEFERRALS + GENUS STUBS at the top of the query — SKIP all every batch:**
     ...
     - `Primula vialii` (ja=サクラソウ) — **NEW deferral A216.** サクラソウ is the name of *Primula sieboldii*; this is likely a mislabel. Verify/fix ja-name before profiling.
   ...
   - **Next batch A217 — from Pseudostellaria / next alpha** (re-run the np=1 query for the fresh top-24/26 + en detail; alphabet in the P→… range — lots of marine inverts, fungi, ferns, moths/beetles, some birds & plants).
   - **A199–A216 done (Claude/Opus, 2026-06-13→14):** +458 profiles (6,148→6,606), 18 batches. Pattern each batch: backup DB → apply `/tmp/fixesA2NN.py` (Python param-binding UPDATEs — handles cultivar quotes & kanji; ja-kanji/romaji→katakana + wrong/lowercase en, always `WHERE scientific_name=…`) → write `/tmp/profile_batchA2NN.py` (robust-rekey template) → run → `seed_species_profiles` → verify table +N & remain −N → commit **sidecar only** → push. DB backups `/tmp/parklife.db.pre_A199…A216` (en/ja-name fixes are LOCAL-only, regenerated into the demo later). Batch sizes 24–26 (taken to a clean genus boundary). ~100 en-name fixes + ~8 ja-name fixes (kanji/romaji→katakana) total. **CAUTION:** count entries before running — confirm `added==len(ENTRIES)` (A200 miscount needed a `…b.py` supplement; A214/A216 were intentionally <26 because of skipped stubs).
