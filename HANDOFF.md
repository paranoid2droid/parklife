...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A198 done)** *(at 6,148 / 2026-06-10; 870 np=1 candidates remaining)*
   - **⚠️ TWO DEFERRED np=1 candidates** sit at the top of the query (SKIP both every batch until resolved):
     ...
     - `Lithospermum murasaki` (ja=ムラサキ, no iNat id) — almost certainly a synonym of `Lithospermum erythrorhizon` (profiled in A198, the canonical ムラサキ/紫草 dye plant). POWO wouldn't confirm cleanly via WebFetch; **next session: verify synonymy (POWO/IPNI) and MERGE into erythrorhizon** (repoint observation.species_id, move/clean aliases, delete park_species + species row, re-run `scripts.dedupe`; backup first) rather than profiling a duplicate.
   - **Next batch A199 — verified clean top-24 (np=1, from DB query, SKIPPING both deferred): Lucilia illustris, Lucilia papuensis, Lucilia sericata, Lucinoma annulata, Luciogobius grandis, Lunella coronata, Luria isabella, Lutjanus fulviflamma, Lutjanus kasmira, Lybia tessellata, Lycocerus japonicus, Lycogala conicum, Lycoperdina mandarinea, Lycopodium clavatum, Lycopus uniflorus, Lycoris × albiflora, Lycosa suzukii, Lygephila lilacina, Lyrodus pedicellatus, Lysimachia decurrens, Lysimachia leucantha, Lythrum hyssopifolia, Macdunnoughia purissima, Machaerina rubiginosa.** (Re-run the np=1 query for fresh en detail. Notables: 3 Lucilia greenbottles (cf. A198 Lucilia ampullacea — Lucilia sericata = green bottle/forensic, en "Blow Fly"→fix); Lutjanus fulviflamma/kasmira = snappers (en "Snapper"/"Moonlighter"→fix to Dory/Common Bluestripe Snapper); Lybia tessellata キンチャクガニ = boxer/pom-pom crab (carries anemones — en "carrying crab"→fix); Luria isabella = Isabel cowrie (en "Isabell cowry"→fix); Lycopodium clavatum ヒカゲノカズラ = stag's-horn clubmoss (spores=石松子); Lycoris × albiflora = white spider lily (toxic Amaryllid); Lysimachia leucantha サワトラノオ = threatened wetland loosestrife; Lycosa suzukii = wolf spider; `Lycoris × albiflora` has `×` in sciname — robust-rekey handles it, verify it matched.)
