...
1. **species_profile curation — np≥3 DONE. np=2 tier in progress (A104–A141 done; next batch continues alphabetically)** *(at 4,908 / 2026-05-31; ~111 np=2 candidates remaining)*
   - **Next batch (A142) start point: around Sparganium erectum / Spiraea cantoniensis onward. Run the Active #1 query (np>=2), dump the 24 rows to a file, READ the full file, and copy names EXACTLY. DO NOT hand-type/reconstruct candidate names — that wasted A135/A137/A140. See the ⚠️ PROCESS LESSON in Status.**
