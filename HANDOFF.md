...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A203 done)** *(at 6,271 / 2026-06-13; 747 np=1 candidates remaining)*
   ...
   - **Next batch A204 — from Muntiacus reevesi** (re-run the np=1 query for fresh top-24/26 + en detail). First candidates seen: Muntiacus reevesi (キョン — ⚠ INVASIVE Reeves's muntjac deer), Musa acuminata (バナナ), Mustela sibirica (シベリアイタチ — Siberian weasel, native+introduced), Mustelus manazo (ホシザメ starspotted smooth-hound), Mutinus borneensis (コイヌノエフデ stinkhorn), 2 Mycena (キクモンクヌギタケ/トサカオチエダタケ bonnet mushrooms)…
   - **A199–A203 done this session (Claude/Opus, 2026-06-13):** +123 profiles (6,148→6,271). Pattern each batch: backup DB → apply `/tmp/fixesA2NN.sql` (ja-kanji→katakana + wrong/lowercase en, always `WHERE scientific_name=…`) → write `/tmp/profile_batchA2NN.py` (robust-rekey template) → run → `seed_species_profiles` → verify table +N & remain −N → commit **sidecar only** → push. DB backups `/tmp/parklife.db.pre_A199…A203` (en/ja-name fixes are LOCAL-only, regenerated into demo later). Batch sizes varied 24–26 (took candidates to a clean genus boundary). Lunella coronata deferral added A199; genus stubs Meretrix/Monstera skipped A202/A203.
