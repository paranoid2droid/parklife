...
- Modal now supports previous/next buttons, keyboard arrows, and touch swipe; gallery export uses iNat `large` URLs and renders via `<img>` at the photo's natural aspect ratio (max-height bounded, no fixed 16:9 crop). Regenerated `docs/index.html` (3.8 MB) and `node --check` passed.
