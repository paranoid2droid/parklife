...
**species_profile np=2 tier IN PROGRESS (A104–A115 done, 4,354 profiled / 2026-05-31; ~664 candidates remaining).** np=2 tier had 952 candidates at start; sweeping alphabetically by `s.scientific_name` from A (A104 Abbottina→Aegista … A114 Coptis quinquefolia→Cycas, A115 Cymodocea→Dipterygina). Next batch **A116**: run Active #1 query with `np >= 2`, take next ~24 by `s.scientific_name` (continues from Dipterygina onward — Disphragis/Dolichovespula…), write `/tmp/profile_batchA116.py`, seed→export→`cp`→commit→push. **Always count your ENTRIES = 24** before running (A113 first wrote 23). **Watch for lowercase-romaji ja-name placeholders** that slip past the `NOT BETWEEN 'A' AND 'Z'` filter (e.g. A114 o-hashibami/sotetsu/furorida…) — fix to proper katakana in DB before profiling. **NB**: the np=2 query returns 24 rows but ~10 already have en+zh in DB yet still lack a species_profile — include ALL 24 (profile is the deliverable; en/zh are sidecar extras), or they re-appear next batch. Local DB en-name fixes during np=2: Aglais io Paecock→European Peacock; Anthus cervinus tree pipit→Red-throated Pipit (A107); Aythya collaris Moon-bill→Ring-necked Duck, Basella alba Spinach→Malabar Spinach, Blattella germanica Crotonbug→German Cockroach (A109); Calendula officinalis Ruddles→Pot Marigold, Calidris pygmaea Spoonbill Sandpiper→Spoon-billed Sandpiper (A110); Caprella scaura Amphipod→Skeleton Shrimp, Catharanthus roseus Oldmaid→Madagascar Periwinkle, Centaurea solstitialis geeldissel(Dutch)→Yellow Star-thistle (A111). Purged junk aliases `竣蜓`+`Lian Zi Cao`. **Code change A104**: added `animal`/`plant` to `GENERIC_EN_NAMES` in `scripts/seed_species_profiles.py:655` so text-mining placeholder en names ("Animal") get corrected by sidecar `common_name_en`.
...
1. **species_profile curation — np≥3 DONE. np=2 tier in progress (A104–A115 done; next A116)** *(at 4,354 / 2026-05-31; ~664 np=2 candidates remaining)*
