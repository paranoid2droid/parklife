...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A199 done)** *(at 6,172 / 2026-06-13; 846 np=1 candidates remaining)*
   - **⚠️ THREE DEFERRED np=1 candidates** sit at the top of the query (SKIP all every batch until resolved):
     - `Latona bicolor` (ja=動物界 junk placeholder; a Donacidae bean clam, iNat 1610898) — needs a verified katakana ja-name before profiling; do NOT profile against 動物界.
     - `Lithospermum murasaki` (ja=ムラサキ, no iNat id) — almost certainly a synonym of `Lithospermum erythrorhizon` (profiled in A198, the canonical ムラサキ/紫草 dye plant). POWO wouldn't confirm cleanly via WebFetch; **verify synonymy (POWO/IPNI) and MERGE into erythrorhizon** (repoint observation.species_id, move/clean aliases, delete park_species + species row, re-run `scripts.dedupe`; backup first) rather than profiling a duplicate.
     - `Lunella coronata` (ja=カンギク — winter-chrysanthemum, a PLANT name wrongly attached to a marine turban shell; en="crowned turban shell") — **ja-name identity MISMATCH**, deferred A199. Verify correct katakana (likely スガイ-group) and fix DB ja-name before profiling; do NOT show カンギク to users.
   - **Next batch A200 — verified clean top-24 (np=1, from DB query, SKIPPING the 3 deferred): Machilus japonica, Machilus kobu, Macrauzata maxima, Macrobrachium formosense, Macrobrachium japonicum, Macrocheira kaempferi, Macrolepiota detersa, Macrophya infumata, Macroptilium lathyroides, Macroscelesia japona, Macrosiagon nasuta, Macrostemum radiatum, Macrotomoxia castanea, Mactromeris polynyma, Maianthemum yesoense, Maiestas dorsalis, Malva mauritiana, Malvastrum coromandelianum, Mangora herbeoides, Marasmiellus crassitunicatus, Marasmius brunneospermus, Marasmius cohaerens, Marasmius maculosus, Marasmius occultatus.** (Re-run the np=1 query for fresh en detail. WATCH ITEMS: `Maianthemum yesoense` ja=`広葉雪笹` is KANJI — fix DB ja-name to katakana `ヒロハユキザサ` before profiling. Junk/wrong en to fix in DB (WHERE scientific_name=…): Macrobrachium formosense "crane river prawn", Macrocheira kaempferi "giant spider crab"→Japanese Spider Crab, Mactromeris polynyma "Arctic surfclam"→cap, Maiestas dorsalis "zig-zag leafhopper"→cap, Malva mauritiana "Mallow", Malvastrum coromandelianum "Broomweed" (this one is legit). Notables: Macrocheira kaempferi タカアシガニ = Japanese spider crab, world's largest arthropod by leg-span; several Marasmius = parachute/horsehair mushrooms.)
   - **A199 notes (Claude):** profiled Lucilia illustris→Machaerotypus sibiricus. Substituted `Machaerotypus sibiricus` for the deferred `Lunella coronata` to keep 24. 6 en-name DB fixes applied (LOCAL-only, /tmp/fixesA199.sql + backup /tmp/parklife.db.pre_A199): Lucilia sericata→Common Green Bottle Fly, Lucinoma annulata→Ringed Lucine, Luria isabella→Isabelle Cowry, Lutjanus fulviflamma→Dory Snapper, Lutjanus kasmira→Common Bluestripe Snapper, Lybia tessellata→Pom-pom Crab.
...
### 2026-06-13 (Claude/Opus) — np=1: A199 (+24 →6,172)
- A199 Lucilia illustris→Machaerotypus sibiricus (24, sidecar-only). 846 np=1 remain. **Next A200 from Machilus japonica — SKIP Latona bicolor + Lithospermum murasaki + Lunella coronata** (3 deferred; list in Active #1).
- **NEW DEFERRAL `Lunella coronata`**: ja=カンギク (winter-chrysanthemum, a plant name) is wrong for a marine turban shell → identity mismatch, deferred; substituted `Machaerotypus sibiricus` to keep batch at 24. Next session: verify katakana + fix DB ja-name.
- Highlights: 3 Lucilia greenbottles (sericata=common green bottle, forensic + maggot-therapy), Lutjanus fulviflamma/kasmira snappers, Lybia tessellata キンチャクガニ (pom-pom/boxer crab — carries anemones, mutualism), Luria isabella cowry, Lycopodium clavatum ヒカゲノカズラ (clubmoss, spores=石松子), Lycoris × albiflora シロバナマンジュシャゲ (⚠ toxic white spider lily), Lysimachia leucantha サワトラノオ + Macdunnoughia silver-Y moth, Lyrodus pedicellatus shipworm (bores ship timber).
- DB fixes (LOCAL-only, /tmp/fixesA199.sql, backup /tmp/parklife.db.pre_A199): 6 en names — Blow Fly→Common Green Bottle Fly, ringed lucine→Ringed Lucine, Isabell cowry→Isabelle Cowry, Snapper→Dory Snapper, Moonlighter→Common Bluestripe Snapper, carrying crab→Pom-pom Crab. Sidecar backfilled 8 common_name_en (Greenbottle/Giant Worm Goby/Japanese Soldier Beetle/Cone Slime Mould/Pleasing Fungus Beetle/Suzuki's Wolf Spider/Lilac Owlet/Forest Loosestrife/Golden Y/Treehopper) + 24 zh aliases. Sidecar pushed; `docs/` demo still lagging (regenerate+push when convenient).
