...
Project is in maintenance + enrichment mode. **461 parks / 9,579 visible species** as of 2026-05-30. Code + Pages site at <https://github.com/paranoid2droid/parklife>; demo published from `docs/` at <https://paranoid2droid.github.io/parklife/> (export ~44 MB). **~7,001 species** have curated profiles in ja/en/zh/zhT (crossed 7,000). **np≥2 tiers 100% CLEARED**. **🎉 np=1 PROFILABLE CANDIDATES NOW CLEARED TOO (A150–A232 done, 2026-06-16).** Every visible np≥1 species with a real katakana ja-name + scientific_name now has a profile. **Only 16 np=1 candidates remain, ALL intentional deferrals/genus-stubs** (un-profilable as-is): deferrals needing a verified katakana ja-name or a decision — Latona bicolor [ja=動物界 junk], Lunella coronata [ja=カンギク mismatch], Primula vialii [ja=サクラソウ mislabel], Ptecticus aurifer [ja=黄色後架虻 Chinese-style], Rhombodera basalis [ja=カマキリ generic], Streptococcus mutans [oral bacterium], Terpnosia nigricosta [ja=セミ placeholder], Wisteriopsis reticulata [ja=サクコウ suspect/unverified — NEW deferral A231]; + genus stubs Meretrix [ハマグリ属], Monstera adansonii [モンステラ属], Platambus [モンキマメゲンゴロウ属], Pseudogobio [カマツカ属], Sympetrum [アカネ属], Tenodera [オオカマキリ属], Tetragnatha [アシナガグモ属], Torularia [ja=植物界 junk]. **(Lithospermum murasaki RESOLVED 2026-06-16 — merged into L. erythrorhizon, already profiled A198.)** **Sidecar data (`data/species_profiles_extra.json`) is pushed & current via the sidecar-only commit workflow.** **🆕 EXPORT SPLIT (2026-06-06, Claude):** the demo dataset is no longer inlined in `docs/index.html`. `export_html` now writes a tiny stable `docs/index.html` (~85 KB) plus a sibling `docs/parklife-data.json` (~46 MB) that the page `fetch()`es at runtime; the two unreferenced dumps (`parklife.json`, `park_species.ndjson`) were dropped from publish. This halves per-commit git bloat and stops index.html churning every batch. **Two consequences:** (1) the 46 MB data file is now the slow-to-push artifact (same throttle caveat that used to apply to index.html — `git add docs/ && commit && push`, retry until a window catches it); (2) `file://` open no longer works — preview via `cd docs && python -m http.server`. No curation data is at risk — all profiles live in the pushed sidecar; the demo just lags until docs/ is pushed. P13 official-URL coverage **443/461 parks (96%)**. Parking: 122 OSM-only + 339 text-confirmed.
...
1. **species_profile curation — np≥2 tiers ALL DONE. 🎉 np=1 PROFILABLE CANDIDATES ALSO CLEARED (A150–A232 done)** *(~7,001 / 2026-06-16; only 16 np=1 left, ALL deferrals/genus-stubs below)*
   - **STATUS: the entire visible-species profile backlog is effectively cleared.** np=1 is the lowest tier (there is no np=0 — species absent from `park_species` aren't queried). The only thing standing between us and 100% are the 16 un-profilable deferrals/stubs below. **Next decision for the user/next agent:** either (a) resolve the deferrals one-by-one (verify each katakana ja-name via iNat/Wikipedia, fix DB, then profile — or decide to leave un-profiled), or (b) pivot to other Active TODOs (item 2 maintenance, or the productization/expansion track in memory). The per-batch profile loop has no more easy fuel.
   - **⚠️ REMAINING 16 = DEFERRALS + GENUS STUBS — each needs a one-off decision, NOT batch profiling:**
     - `Latona bicolor` (ja=動物界 junk; Donacidae bean clam, iNat 1610898) — needs verified katakana ja-name; do NOT profile against 動物界.
     - `Lunella coronata` (ja=カンギク = winter-chrysanthemum on a marine turban shell) — ja-name MISMATCH. Verify katakana (likely スガイ-group) + fix DB ja-name before profiling.
     - `Primula vialii` (ja=サクラソウ) — deferral A216. サクラソウ is the name of *Primula sieboldii*; likely a mislabel. Verify/fix ja-name before profiling.
     - `Ptecticus aurifer` (ja=黄色後架虻) — deferral A217. Chinese-style name (a soldier fly, Stratiomyidae); needs a verified katakana ja-name before profiling.
     - `Rhombodera basalis` (ja=カマキリ) — deferral A219. カマキリ is the generic Japanese word for "mantis" applied to a specific exotic giant shield mantis (SE Asia); needs a verified katakana name (likely ヒロハネカマキリ) before profiling.
     - `Wisteriopsis reticulata` (ja=サクコウ) — **NEW deferral A231.** Evergreen wisteria (=Millettia reticulata); サクコウ is an unverified/suspect ja-name (could not confirm — possibly a placeholder). Verify katakana before profiling.
     - `Streptococcus mutans` (ja=ミュータンス菌) — oral bacterium, not park biota; leave un-profiled / candidate for deletion.
     - `Terpnosia nigricosta` (ja=セミ placeholder) — generic "cicada"; needs a verified katakana name.
     - **Genus-level stubs (属) — skip permanently:** `Meretrix` (ハマグリ属), `Monstera adansonii` (モンステラ属), `Platambus` (モンキマメゲンゴロウ属), `Pseudogobio` (カマツカ属), `Sympetrum` (アカネ属), `Tenodera` (オオカマキリ属), `Tetragnatha` (アシナガグモ属), `Torularia` (ja=植物界 junk). (Real species in these genera were profiled normally.)
   - **✅ Lithospermum murasaki RESOLVED (Claude, 2026-06-16):** merged into `Lithospermum erythrorhizon` (id 9023, profiled A198). Repointed observation, moved aliases (kept `Lithospermum murasaki`/sci as a findable alias), deleted park_species + species row 418, re-ran `scripts.dedupe`. Backup `/tmp/parklife.db.pre_merge_murasaki`. Method model for the other merges if needed.
