...
Project is in maintenance + enrichment mode. **461 parks / 9,579 visible species** as of 2026-05-30. Code + Pages site at <https://github.com/paranoid2droid/parklife>; demo published from `docs/` at <https://paranoid2droid.github.io/parklife/> (export ~44 MB). **~6,966 species** have curated profiles in ja/en/zh/zhT (crossed 6,000). **np≥2 tiers 100% CLEARED**. **np=1 tier IN PROGRESS** (A150–A230 done; **52 np=1 candidates remaining**; next A231 from Viola diffusa / next-alpha (V… range) — SKIP the deferrals + genus stubs at top of query: Latona bicolor, Lithospermum murasaki, Lunella coronata, Primula vialii, Ptecticus aurifer, Rhombodera basalis, Streptococcus mutans [oral bacterium], Terpnosia nigricosta [ja=セミ placeholder] [all deferred], + genus stubs Meretrix [ハマグリ属], Monstera adansonii [モンステラ属], Platambus [モンキマメゲンゴロウ属], Pseudogobio [カマツカ属], Sympetrum [アカネ属], Tenodera [オオカマキリ属], Tetragnatha [アシナガグモ属], Torularia [ja=植物界 junk]). ⚠ **NEXT-BATCH WATCH:** Viola odorata has a romaji ja-name (nioi-sumire) → fix to ニオイスミレ in DB before profiling. **Sidecar data (`data/species_profiles_extra.json`) is pushed & current via the sidecar-only commit workflow.** **🆕 EXPORT SPLIT (2026-06-06, Claude):** the demo dataset is no longer inlined in `docs/index.html`. `export_html` now writes a tiny stable `docs/index.html` (~85 KB) plus a sibling `docs/parklife-data.json` (~46 MB) that the page `fetch()`es at runtime; the two unreferenced dumps (`parklife.json`, `park_species.ndjson`) were dropped from publish. This halves per-commit git bloat and stops index.html churning every batch. **Two consequences:** (1) the 46 MB data file is now the slow-to-push artifact (same throttle caveat that used to apply to index.html — `git add docs/ && commit && push`, retry until a window catches it); (2) `file://` open no longer works — preview via `cd docs && python -m http.server`. No curation data is at risk — all profiles live in the pushed sidecar; the demo just lags until docs/ is pushed. P13 official-URL coverage **443/461 parks (96%)**. Parking: 122 OSM-only + 339 text-confirmed.
...
1. **species_profile curation — np≥2 tiers ALL DONE. np=1 tier IN PROGRESS (A150–A230 done)** *(~6,966 / 2026-06-15; 52 np=1 candidates remaining)*
   - **⚠️ DEFERRALS + GENUS STUBS at the top of the query — SKIP all every batch:**
     - `Latona bicolor` (ja=動物界 junk; Donacidae bean clam, iNat 1610898) — needs verified katakana ja-name; do NOT profile against 動物界.
     - `Lithospermum murasaki` (ja=ムラサキ, no iNat id) — almost certainly synonym of `Lithospermum erythrorhizon` (profiled A198). **Verify synonymy and MERGE into erythrorhizon** (repoint observation.species_id, move/clean aliases, delete park_species + species row, re-run `scripts.dedupe`; backup first).
     ...
     - `Ptecticus aurifer` (ja=黄色後架虻) — deferral A217. That is a Chinese-style name (a soldier fly, Stratiomyidae); needs a verified katakana ja-name before profiling.
     - `Rhombodera basalis` (ja=カマキリ) — **NEW deferral A219.** カマキリ is the generic Japanese word for "mantis" applied to a specific exotic giant shield mantis (SE Asia); needs a verified katakana name (likely ヒロハネカマキリ) before profiling.
     - **Genus-level stubs (属) — skip permanently:** `Meretrix` (ハマグリ属), `Monstera adansonii` (モンステラ属), `Platambus` (モンキマメゲンゴロウ属), `Pseudogobio` (カマツカ属). (Real species in these genera were profiled normally.)
   - **Next batch A231 — from Viola diffusa / next alpha** (re-run the np=1 query for the fresh top-26 + en detail; alphabet now V→Z range — only 52 candidates left, so the np=1 tier finish is in sight). ⚠ Viola odorata = romaji ja `nioi-sumire`, fix to ニオイスミレ in DB first. /tmp templates from this session are gone after `/clear`; rebuild from any committed `/tmp/profile_batchA230.py`-style model (robust-rekey + indent=2 write; run with `PYTHONPATH=/Users/zhe/ClaudeCode/parklife` or `sys.path.insert`).
