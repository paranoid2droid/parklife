...
**species_profile np=2 tier IN PROGRESS (A104–A109 done, 4,210 profiled / 2026-05-31; ~808 candidates remaining).** np=2 tier had 952 candidates at start; sweeping alphabetically by `s.scientific_name` from A (A104 Abbottina→Aegista, A105 Aeshna→Amata, A106 Amelanchier→Anthemis, A107 Anthonomus→Argyrodes, A108 Arichanna→Atractomorpha, A109 Auletobius→Blattella). Next batch **A110**: run Active #1 query with `np >= 2`, take next ~24 by `s.scientific_name` (continues from Blattella onward — Blechnum/Boletus…), write `/tmp/profile_batchA110.py`, seed→export→`cp`→commit→push. Local DB en-name fixes during np=2: `Aglais io` Paecock→European Peacock; `Anthus cervinus` tree pipit→Red-throated Pipit (A107, was mislabeled — tree pipit is A. trivialis); `Aythya collaris` Moon-bill→Ring-necked Duck, `Basella alba` Spinach→Malabar Spinach, `Blattella germanica` Crotonbug→German Cockroach (A109). Purged junk aliases `竣蜓`+`Lian Zi Cao`. **Code change A104**: added `animal`/`plant` to `GENERIC_EN_NAMES` in `scripts/seed_species_profiles.py:655` so text-mining placeholder en names ("Animal") get corrected by sidecar `common_name_en`.
...
1. **species_profile curation — np≥3 DONE. np=2 tier in progress (A104–A109 done; next A110)** *(at 4,210 / 2026-05-31; ~808 np=2 candidates remaining)*
