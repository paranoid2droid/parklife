...
1. **species_profile curation — np≥2 tiers ALL DONE (A1–A149). Next: np=1 long tail (~1,994 candidates)** *(at 5,024 / 2026-06-01; np=2 verified 0 remaining)*
   - **Next batch (A150+) — np=1 tier, restart alphabetically from A.** Same Active #1 query but `HAVING COUNT(DISTINCT ps.park_id) = 1`. Dump 24 rows to a file, READ the full file, copy names EXACTLY (reconstructing from memory wasted A135/A137/A140/A142/A147/A148 — robust-rekey prevented DB pollution but the batches did nothing). Reuse `/tmp/finish_generic.sh` + `/tmp/profile_batchA149.py` model. Verify profile_count +N AND remain −N each batch. **np=1 ≈ 1,994 species / ~83 batches — a multi-session marathon; pace it, keep HANDOFF current.**
