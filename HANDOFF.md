...
1. **species_profile curation — 🎉🎉 COMPLETE. Entire profilable backlog cleared (A1–A234 done).** *(7,011 / 2026-06-17; only 7 genus-level 属 stubs remain — irreducible)*
   - **STATUS: DONE. There is no more per-batch profiling fuel.** Every visible species with a real katakana ja-name + scientific_name is profiled in 4 languages. All np tiers (≥2, then =1) cleared, and all 8 content deferrals resolved 2026-06-17 (see Recent sessions for the verified names). **The only remaining np=1 query hits are 7 genus-level stubs** — `Meretrix` (ハマグリ属), `Monstera adansonii` (モンステラ属), `Platambus` (モンキマメゲンゴロウ属), `Pseudogobio` (カマツカ属), `Sympetrum` (アカネ属), `Tenodera` (オオカマキリ属), `Tetragnatha` (アシナガグモ属). These are genus-only records (the source page named only the 属); a species profile is not meaningful, so they stay as-is and display their real Japanese genus name. **Do NOT keep re-running the np query expecting new candidates.**
...
- **Photo gallery** *(8,853 / 9,577 visible species = 92.4% coverage, verified 2026-06-20)*
  - ~724 visible species still have no photo. Periodically re-run `scripts.collect_species_photos` (broad fallback) as iNat acquires more.
