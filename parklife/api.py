    ...
    with _connect() as conn:
        ...
        rows = conn.execute(
            ...
            SELECT s.id, s.scientific_name, s.common_name_ja, s.common_name_en,
                   ...
                   ps.evidence_tier, ps.abundance, ps.last_year, ps.observer_count
        ...
        for r in rows:
            card = {
                ...
                "ly": r["last_year"] or 0,   # most recent observation year (recency)
                "ob": r["observer_count"] or 0,  # distinct observers (confidence)
            }
